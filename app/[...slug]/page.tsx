...
function WorkPage() {
  const projects = [
    ['Enterprise learning platform', 'Healthcare · Learning', 'Scalable learning experiences for healthcare professionals, combining reusable web components, Node.js APIs, SQL data, and AWS delivery.'],
    ['AI-assisted image annotation platform', 'AI · Enterprise', 'Led full-stack delivery for an AI-assisted microbial detection workflow, including enterprise features and Azure DevOps CI/CD.'],
    ['Healthcare communication portal', 'Healthcare · Cloud', 'Secure web modules connected to cloud services including EC2 and Lambda, built for reliable communication workflows.'],
    ['Enterprise ecommerce platform', 'Commerce · APIs', 'Modular frontend experiences consuming REST APIs, designed for maintainable product delivery at scale.'],
  ];
  return <Shell><Intro kicker="Selected work" title={<>Proof over<br /><em>promises.</em></>} copy="A public-safe view of the product domains, technical responsibilities, and delivery environments I have worked across." /><section className="case-study-grid">{projects.map(([title, tag, copy], index) => <article className="case-study" key={title}><span className="case-number">0{index + 1}</span><p className="eyebrow">{tag}</p><h2>{title}</h2><p>{copy}</p><span className="case-role">Architecture · Development · Delivery</span></article>)}</section><section className="split-callout"><p className="eyebrow">How I contribute</p><h2>From first<br /><em>diagram to delivery.</em></h2><p>I translate product requirements into maintainable systems, build across the stack, integrate APIs and data, and support cloud delivery and team execution.</p></section></Shell>;
}
...
function ServicesPage() {
  ...
  return <Shell><Intro kicker="Capabilities" title={<>Useful skills,<br /><em>carefully applied.</em></>} copy="An informational view of the capabilities I bring to product teams and engineering environments. Availability for outside work depends on employer policies and commitments." /><section className="service-list">{services.map(([number, title, copy]) => <div className="service-item" key={number}><span className="service-number">{number}</span><div><h2>{title}</h2><p>{copy}</p></div><span className="service-arrow" aria-hidden="true">↗</span></div>)}</section></Shell>;
}
