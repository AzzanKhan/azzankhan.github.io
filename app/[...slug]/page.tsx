...
function WorkPage() {
  const projects = [
    { title: 'E-Learning Platform for Doctors and Nurses', client: 'Client: Elsevier · associated with CitiusTech', dates: 'Jan 2025 — present', tag: 'Healthcare · Learning', copy: 'Developing a scalable learning platform for healthcare professionals. Built reusable React components, scalable Node.js APIs, SQL data integrations, and supported AWS deployments.' },
    { title: 'Microbial Detection Application', client: 'Client: GSK · associated with CitiusTech', dates: 'Jan 2024 — Dec 2024', tag: 'AI · Healthcare', copy: 'Led full-stack development of an AI-assisted image annotation platform, implemented enterprise features, and managed Azure DevOps CI/CD pipelines.' },
    { title: 'Communication Portal', client: 'Client: GE Healthcare · associated with CitiusTech', dates: 'Aug 2022 — Dec 2023', tag: 'Healthcare · Cloud', copy: 'Led React and Node.js-based development for a healthcare communication portal, integrating AWS EC2 and Lambda functions.' },
    { title: 'Data Visualization Tool', client: 'Client: Roche · associated with CitiusTech', dates: 'Jan 2022 — Jul 2022', tag: 'Healthcare · Data', copy: 'Delivered a MEAN stack solution that supported clearer decision-making through focused data visualization experiences.' },
    { title: 'Ecommerce Platform', client: 'Client: Belgium Post · associated with Infosys', dates: 'Jul 2021 — Dec 2021', tag: 'Commerce · APIs', copy: 'Built a dynamic, modular Angular frontend consuming REST APIs for a large postal and ecommerce experience.', url: 'https://www.bpost.be/en' },
    { title: 'Cloud Migration to AWS', client: 'Client: Belgium Post · associated with Infosys', dates: 'Sep 2019 — Jun 2021', tag: 'Cloud · Integration', copy: 'Developed integrations using AWS Lambda, S3, and Step Functions as part of a cloud migration programme.', url: 'https://www.bpost.be/en' },
  ];
  return <Shell><Intro kicker="Selected work" title={<>Proof over<br /><em>promises.</em></>} copy="A public-safe view of product problems, responsibilities, and delivery environments from my time associated with CitiusTech and Infosys. I was a contributor on these programmes, not the owner of the client products." /><section className="case-study-grid">{projects.map(({ title, client, dates, tag, copy, url }, index) => <article className="case-study" key={title}><span className="case-number">0{index + 1}</span><p className="eyebrow">{tag}</p><h2>{title}</h2><p className="case-client">{client}</p><p className="case-dates">{dates}</p><p>{copy}</p><span className="case-role">Architecture · Development · Delivery</span>{url && <a className="case-link" href={url} target="_blank" rel="noreferrer">View public organisation site {arrow}</a>}</article>)}</section><section className="split-callout"><p className="eyebrow">How I contribute</p><h2>From first<br /><em>diagram to delivery.</em></h2><p>I translate product requirements into maintainable systems, build across the stack, integrate APIs and data, and support cloud delivery and team execution.</p></section></Shell>;
}
