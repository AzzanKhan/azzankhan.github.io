...
function Header() {
  return <header className="site-header"><Link className="logo" href="/"><span className="logo-mark">AK</span><span>Azzan Khan</span></Link><nav aria-label="Primary navigation">{navItems.map(([label, href]) => <Link key={href} href={href}>{label}</Link>)}</nav><Link className="button button-small button-dark" href="/contact">Let&apos;s talk {arrow}</Link><button className="menu-toggle" aria-label="Menu">☰</button></header>;
}

function Footer() {
  return <footer className="site-footer"><div><Link className="logo" href="/"><span className="logo-mark">AK</span><span>Azzan Khan</span></Link><p className="muted">Lead Engineer · Full-Stack Developer · Builder</p></div><div className="footer-links"><a href="https://www.youtube.com/" target="_blank" rel="noreferrer">YouTube {arrow}</a><a href="mailto:hello@azzankhan.dev">Email {arrow}</a><Link href="/contact">Contact {arrow}</Link></div><p className="copyright">© {new Date().getFullYear()} Azzan Khan. Built with intent.</p></footer>;
}
...
export function Home() {
  return <Shell><section className="hero"><div className="hero-copy"><p className="eyebrow">{dot}Independent engineer · India</p><h1>I turn complex ideas into <em>useful</em> digital products.</h1><p className="hero-lead">I&apos;m Azzan — a lead engineer and full-stack developer who enjoys the space between a clear product idea and a well-crafted launch.</p><div className="hero-actions"><Link className="button button-dark" href="/work">See my work {arrow}</Link><Link className="text-link" href="/about">More about me {arrow}</Link></div></div><div className="hero-visual"><div className="orb orb-one" /><div className="orb orb-two" /><div className="code-card"><span className="code-label">currently_building</span><code><b>const</b> goodWork = {'{'}<br />&nbsp;&nbsp;clarity: <i>true</i>,<br />&nbsp;&nbsp;craft: <i>true</i>,<br />&nbsp;&nbsp;momentum: <i>true</i><br />{'}'};</code></div><div className="hero-note">Systems thinker<br /><strong>with a maker&apos;s bias.</strong></div></div></section><section className="trusted section-rule"><p className="eyebrow">A few things I care about</p><div className="principles"><div><strong>01</strong><span>Make it clear</span></div><div><strong>02</strong><span>Make it useful</span></div><div><strong>03</strong><span>Make it last</span></div></div></section><section className="section"><div className="section-heading"><div><p className="eyebrow">{dot}Selected work</p><h2>Building for people,<br /><em>not just pixels.</em></h2></div><Link className="text-link" href="/work">View all work {arrow}</Link></div><div className="work-grid"><Link className="work-card work-card-large" href="/work"><div className="work-card-art garden-art"><span>Garden<br /><i>Public School</i></span><small>Education · Digital experience</small></div><div className="work-card-caption"><strong>Garden Public School</strong><span>01 / Featured work</span></div></Link><Link className="work-card" href="/projects"><div className="work-card-art systems-art"><span>Systems<br /><i>that scale</i></span><small>Engineering · Product thinking</small></div><div className="work-card-caption"><strong>Product &amp; platform work</strong><span>02 / Ongoing</span></div></Link></div></section><section className="section writing-section"><div className="section-heading"><div><p className="eyebrow">{dot}From the archive</p><h2>Notes on the<br /><em>craft of building.</em></h2></div><Link className="text-link" href="/blog">Browse all writing {arrow}</Link></div><div className="post-grid">{posts.slice(0, 3).map((post) => <PostCard key={post.slug} post={post} />)}</div></section><section className="video-banner"><div><p className="eyebrow">Watch along</p><h2>Build in public.<br /><em>Learn out loud.</em></h2><p>Practical videos on engineering, architecture, and the lessons hiding in the work.</p></div><Link className="button button-light" href="/youtube">Visit YouTube {arrow}</Link></section><section className="cta-section"><p className="eyebrow">{dot}Have a good problem?</p><h2>Let&apos;s make something<br /><em>worth making.</em></h2><Link className="button button-dark" href="/contact">Start a conversation {arrow}</Link></section></Shell>;
}
...
function StandardPage({ kind }: { kind: string }) {
  ...
  return <Shell><Intro {...item} /><section className="split-callout"><p className="eyebrow">Azzan Khan</p><h2>{kind === 'contact' ? <>Let&apos;s build<br /><em>something useful.</em></> : <>Less theatre.<br /><em>More progress.</em></>}</h2><p>Clear questions, small iterations, and enough care in the details to make the final thing feel inevitable.</p><Link className="button button-dark" href={kind === 'contact' ? 'mailto:hello@azzankhan.dev' : '/contact'}>{kind === 'contact' ? 'Email me' : 'Start a conversation'} {arrow}</Link></section></Shell>;
}
